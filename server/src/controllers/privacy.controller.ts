 ...
 */
export const getUserConsents = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      res.status(401).json({
 ...
 */
export const checkRequiredConsents = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      res.status(401).json({
...
export const updatePrivacySettings = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    if (
      ...
      !['public', 'contacts', 'private'].includes(settings.profile_visibility)
    ) {
      res.status(400).json({
        success: false,
...
export const exportData = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename="kognirecovery-data-${userId}.json"`
    );
 ...
 */
export const getRetentionPolicies = async (_req: Request, res: Response): Promise<void> => {
  try {
