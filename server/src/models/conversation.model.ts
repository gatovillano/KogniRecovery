...
export const createConversation = async (
  ...
  const result = await query<Conversation>(sql, values);
...
export const getActiveConversation = async (userId: string): Promise<Conversation | null> => {
  const sql = `
    ...
    LIMIT 1`;
  const result = await query<Conversation>(sql, [userId]);
  return result.rows[0] || null;
};
...
export const getUserConversations = async (
  ...
  const sql = `
    ...
    ORDER BY last_message_at DESC 
    LIMIT $2 OFFSET $3`;
  const result = await query<Conversation>(sql, [userId, limit, offset]);
...
export const closeConversation = async (
  ...
  const result = await query<Conversation>(sql, [id, satisfaction, helpfulness, relevance]);
  return result.rows[0] || null;
};
...
export const createConversationSession = async (
  ...
  const result = await query<ConversationSession>(sql, [conversationId, userId, sessionNumber]);
  return result.rows[0] || null;
};
...
export const getSessionById = async (id: string): Promise<ConversationSession | null> => {
  ...
  const result = await query<ConversationSession>(sql, [id]);
  return result.rows[0] || null;
};
...
export const getConversationSessions = async (
  ...
  const result = await query<ConversationSession>(sql, [conversationId]);
  return result.rows;
};
...
export const updateSession = async (
  ...
  for (const [key, value] of Object.entries(data)) {
    if (allowedFields.includes(key) && value !== undefined) {
      ...
      if (
        ...
        key === 'strategies_shared' ||
        key === 'resources_recommended'
      ) {
        values.push(JSON.stringify(value));
      } else {
        values.push(value);
  ...
  const result = await query<ConversationSession>(sql, values);
  return result.rows[0] || null;
};
...
export const closeSession = async (
  ...
  const result = await query<ConversationSession>(sql, [outcome, id]);
  return result.rows[0] || null;
};
...
export const getScenarioConfig = async (scenarioType: string): Promise<ScenarioConfig | null> => {
  ...
  const result = await query<ScenarioConfig>(sql, [scenarioType]);
  return result.rows[0] || null;
};
...
export const getActiveScenarios = async (): Promise<ScenarioConfig[]> => {
  ...
  const result = await query<ScenarioConfig>(sql, []);
  return result.rows;
};
...
export const detectAppropriateScenario = async (
  ...
  // Detectar craving
  if (
    lowerMessage.includes('ganas de consumir') ||
  ...
  if (
    ...
    lowerMessage.includes('no tengo ganas') ||
...
export const getConversationStats = async (
  ...
  const sessionsSql = `
     ...
     WHERE c.user_id = $1`;
  const sessionsResult = await query<{ total: string }>(sessionsSql, [userId]);

  return {
    totalConversations: parseInt(row.total || '0', 10),
    activeConversations: parseInt(row.active || '0', 10),
    completedConversations: parseInt(row.completed || '0', 10),
    averageMessages: parseFloat(row.avg_messages || '0'),
    averageDuration: parseFloat(row.avg_duration || '0'),
    totalSessions: parseInt(sessionsResult.rows[0]?.total || '0', 10),
  };
};
